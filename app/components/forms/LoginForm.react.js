...
const submit = async (values, dispatch) => { // eslint-disable-line consistent-return, max-len
  try {
    const response = await login(values.username, values.password)(dispatch); //(dispatch),函数执行后监听回调
...
const validate = (values, props) => {  // eslint-disable-line no-unused-vars
  const errors = {};
  if (!values.username) errors.username = 'username error'; //username为空时
  if (!values.password) errors.password = 'password error'; //password为空时
  return errors;
};
